...
export const getMySendComment = (page, okCallback, errorCallback) => {

    if (DEBUG) {
        setTimeout(function () {
            okCallback(data.sendcomment)
        }, 1500)
        return
    }
    ...
    var config = {
        method: 'get',
        url: API_ROUTER_CONFIG.send_comment,
    ...
    axios(config)
        .then(function (response) {
            logger("oauthPost-ok", 'getMySendComment response succeed')
